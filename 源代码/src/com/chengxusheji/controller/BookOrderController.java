...
public class BookOrderController extends BaseController {
	...
	public void add(@Validated BookOrder bookOrder, BindingResult br,
		...
		int sellId = bookOrder.getBookSellObj().getSellId();
		String sellUserName = bookSellService.getBookSell(sellId).getUserObj().getUser_name();
		 
		String orderUserName = bookOrder.getUserObj().getUser_name();
		...
		bookOrder.setAddTime(sdf.format(new java.util.Date()));
	...
	public String frontUserlist(@ModelAttribute("bookSellObj") BookSell bookSellObj,@ModelAttribute("userObj") UserInfo userObj,String addTime,Integer currentPage, Model model, HttpServletRequest request,HttpSession session) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (addTime == null) addTime = "";
	    ...
	    List<UserInfo> userInfoList = userInfoService.queryAllUserInfo();
	    request.setAttribute("userInfoList", userInfoList);
		return "BookOrder/bookOrder_frontUserquery_result"; 
	}
