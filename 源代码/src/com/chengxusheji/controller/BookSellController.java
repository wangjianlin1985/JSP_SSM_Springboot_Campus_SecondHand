...
public class BookSellController extends BaseController {
	...
	public void userAdd(@Validated BookSell bookSell, BindingResult br,
		...
		bookSell.setAddTime(sdf.format(new java.util.Date()));
	...
	public String fronUsertlist(String bookName,@ModelAttribute("bookClassObj") BookClass bookClassObj,String publish,String author,String xjcd,@ModelAttribute("userObj") UserInfo userObj,String addTime,Integer currentPage, Model model, HttpServletRequest request,HttpSession session) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (bookName == null) bookName = "";
		if (publish == null) publish = "";
		if (author == null) author = "";
	    ...
	    List<UserInfo> userInfoList = userInfoService.queryAllUserInfo();
	    request.setAttribute("userInfoList", userInfoList);
		return "BookSell/bookSell_frontUserquery_result"; 
	}
	...
	public String frontshow(@PathVariable Integer sellId,Model model,HttpServletRequest request) throws Exception {
        ...
        ArrayList<BookOrder> bookOrderList = bookOrderService.queryBookOrder(bookSell, null, "");
        List<BookClass> bookClassList = bookClassService.queryAllBookClass();
        request.setAttribute("bookClassList", bookClassList);
        List<UserInfo> userInfoList = userInfoService.queryAllUserInfo();
        request.setAttribute("userInfoList", userInfoList);
